...
type CreateCollectionTaskRequest struct {
	...
	DataTypes    []string `json:"dataTypes" validate:"required,dive,oneof=clusters hosts vms metrics"`
	MetricsDays  int      `json:"metricsDays" validate:"min=1,max=365"`
...
type CreateAnalysisTaskRequest struct {
	ConnectionID  uint                   `json:"connectionId" validate:"required"`
	AnalysisTypes []string               `json:"analysisTypes" validate:"required,dive,oneof=zombie rightsize tidal health"`
...
type GetMetricsRequest struct {
	VMID       uint   `json:"vmId" validate:"required"`
	MetricType string `json:"metricType" validate:"required,oneof=cpu memory disk_read disk_write net_rx net_tx"`
