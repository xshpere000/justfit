"""JustFit API End-to-End Test.

Tests complete API workflow following docs/API_TEST_README.md:
1. Create connection → test connection
2. Get connection list & details
3. Create collection task
4. Wait for collection completion
5. Get collection results
6. Idle detection analysis
7. Resource analysis
8. Health score analysis
9. Generate Excel report
10. Generate PDF report
11. Delete connection

Run with: python3.14 tests/backend/e2e/test_api_e2e.py
"""
...
class APIE2ETest:
    ...
    def __init__(self, base_url: str = BASE_URL):
        ...
        self.vcenter_config = {
            "name": f"E2E_Test_vCenter_{uuid.uuid4().hex[:8]}",
            "platform": "vcenter",
            "host": "10.103.116.116",
            "port": 443,
    ...
    async def test_01_create_connection(self) -> bool:
        ...
        try:
            ...
            test_results["passed"].append("用例 1: 创建连接")
            return True

        except AssertionError as e:
            self._log(f"✗ Test failed: {e}", "ERROR")
            test_results["failed"].append(f"用例 1: 创建连接 - {e}")
            return False
        except Exception as e:
    ...
    async def test_02_test_connection(self) -> bool:
        ...
        if not self.connection_id:
            ...
            test_results["skipped"].append("用例 2: 验证连接")
            return False
        ...
        try:
            ...
            test_results["passed"].append("用例 2: 验证连接")
            return True

        except AssertionError as e:
            self._log(f"✗ Test failed: {e}", "ERROR")
            test_results["failed"].append(f"用例 2: 验证连接 - {e}")
            return False
        except Exception as e:
            self._log(f"✗ Unexpected error: {e}", "ERROR")
            test_results["failed"].append(f"用例 2: 验证连接 - {e}")
    ...
    async def test_03_get_connections(self) -> bool:
        ...
        try:
            ...
            assert data["success"] is True, "success should be True"
            assert "data" in data, "Response should have data field"
            assert isinstance(data["data"], list) or "items" in data["data"], "data should be list or have items"
    ...
    async def test_05_create_task(self) -> bool:
        ...
        try:
            task_config = {
                "name": "E2E_vCenter_Basic_Collection",
                "type": "collection",
    ...
    async def test_06b_create_metrics_task(self) -> bool:
        ...
        try:
            ...
            task_config = {
                "name": "E2E_vCenter_Metrics_Collection",
                "type": "collection",
                "connectionId": self.connection_id,
                "config": {
    ...
    async def test_08_idle_detection(self) -> bool:
        ...
        try:
            ...
            # Allow 200 or 202 for async processing
    ...
    async def test_09_resource_analysis(self) -> bool:
        ...
        try:
            ...
            if "rightSize" in resource_data:
                ...
                if isinstance(right_size, list) and right_size:
                    for item in right_size[:2]:
                        self._log(f"    - {item.get('vmName', 'N/A')}: waste {item.get('wasteRatio', 0)*100:.1f}%")
    ...
    async def test_11_generate_excel_report(self) -> bool:
        ...
        try:
            ...
            # Accept 200, 201, or 202 for async processing
    ...
    async def test_14c_create_task_with_metric_days(self) -> bool:
        ...
        try:
            ...
            for days in metric_days_list:
                task_config = {
                    "name": f"E2E_Task_MetricDays_{days}",
                    "type": "collection",
                    "connectionId": self.connection_id,
                    "metricDays": days,
                    "config": {
    ...
    async def test_14d_update_task_mode(self) -> bool:
        ...
        try:
            # 先创建一个任务
            task_config = {
                "name": "E2E_Task_Mode_Update_Test",
                "type": "collection",
                "connectionId": self.connection_id,
                "mode": "safe",
                "config": {
    ...
    async def test_14_delete_connection(self) -> bool:
        ...
        try:
            ...
            test_results["passed"].append("用例 14: 删除连接")
            return True
...
async def run_all_tests():
    """运行所有测试用例"""
    print("\n" + "=" * 60)
    print("JustFit API End-to-End Test")
