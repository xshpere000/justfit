...
// 前端调用 analysisType: zombie, rightsize, tidal, health
// 后端转换为: zombie_vm, right_size, tidal, health_score
type CreateAnalysisTaskRequest struct {
    AnalysisType string                 `json:"analysis_type" binding:"required"` // zombie, rightsize, tidal, health
    ConnectionID uint                   `json:"connection_id" binding:"required"`
    Config       map[string]interface{} `json:"config"`
}

// 返回任务ID
type CreateAnalysisTaskResponse struct {
...
type GetTaskAnalysisResultRequest struct {
    TaskID       uint   `json:"task_id" binding:"required"`
    AnalysisType string `json:"analysis_type"` // 为空则返回所有类型
}

type TaskAnalysisResult struct {
    ZombieVM   *ZombieVMResult   `json:"zombie_vm,omitempty"`   // 僵尸VM分析结果
    RightSize  *RightSizeResult  `json:"right_size,omitempty"`  // Right Size分析结果
    Tidal      *TidalResult      `json:"tidal,omitempty"`       // 潮汐检测结果
    HealthScore *HealthScoreResult `json:"health_score,omitempty"` // 健康评分结果
}

type ZombieVMResult struct {
    Count   int                    `json:"count"`
    Results []ZombieVMEntry        `json:"results"`
}

type RightSizeResult struct {
    Count   int                    `json:"count"`
    Results []RightSizeEntry        `json:"results"`
}

type TidalResult struct {
    Count   int                    `json:"count"`
    Results []TidalEntry            `json:"results"`
}
...
type HealthScoreResult struct {
    ...
    OvercommitRisk       string   `json:"overcommit_risk"`
    HotspotConcentration string   `json:"hotspot_concentration"`
